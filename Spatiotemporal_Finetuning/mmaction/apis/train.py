...
from mmcv.runner import (DistSamplerSeedHook, EpochBasedRunner, OptimizerHook,
                         build_optimizer, get_dist_info, load_state_dict)
...
def train_model(model,
    ...
    if cfg.resume_from:
        ...
        # inflate
        if cfg.inflate:
            state_dict = inflate_3d_state_dict(cfg.load_from,  revise_keys=[('get_feature', 'backbone'),(r'^fc1\.', 'cls_head.fc1.')])
            load_state_dict(runner.model, state_dict, logger=logger)
        else:
            print('no inflate')
            #('classifier', 'cls_head.fc_cls')
            #('classifier.0.', 'cls_head.fc1.')
            #('classifier.2.', 'cls_head.fc1.')
            #(r'^fc1\.', 'cls_head.fc1.'),('classifier.0.', 'cls_head.fc1.'),('classifier.2.', 'cls_head.fc2.')
            runner.load_checkpoint(cfg.load_from,  revise_keys=[('get_feature', 'backbone'),(r'^fc1\.', 'cls_head.fc1.'), ('classifier.0.', 'cls_head.fc1.'), ('classifier', 'cls_head.fc_cls')])
       # runner.load_checkpoint(cfg.load_from)

   # for i in runner.model.module.named_parameters():
   #        print(i)
    runner_kwargs = dict()
    if cfg.omnisource:
...
def inflate_3d_state_dict(path, revise_keys):
    print('inflate the 3D kernel')
    ...
    inflate_dict = OrderedDict()
    for k, v in state_dict.items():
        if len(v.shape) == 5 and v.size(2) == 3:
            res =   v[:, :, 1:2, :, :] /3
            inflation_rate = [1/3, 1, 1/3]
            for i in range(3):
                v[:, :, i:i+1, :, :] = res * inflation_rate[i]
        inflate_dict[k] = v
    return inflate_dict

def inflate_motion_state_dict(path, revise_keys):
    state_dict = torch.load(path, map_location='cpu')['state_dict']
    metadata = getattr(state_dict, '_metadata', OrderedDict())
    for p, r in revise_keys:
        state_dict = OrderedDict(
    ...
    for k, v in state_dict.items():
        if len(v.shape) == 5 and v.size(2) == 3:
            res = v
            app_para, motion_para = torch.chunk(v, 2, dim=0)
            motion_kernel_chunks = torch.chunk(motion_para, 3, dim=0)
            for i in range(3):
                for j in range(3):
                    motion_kernel_chunks[i][:, :, j:j+1, :, :] = motion_kernel_chunks[i][:, :, i:i+1, :, :] / 3
            motion_para = torch.cat(list(motion_kernel_chunks), dim=0)
            v = torch.cat([app_para, motion_para], dim=0)
            print(v == res)
    return state_dict