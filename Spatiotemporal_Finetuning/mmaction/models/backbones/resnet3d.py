...
from .STS_Conv import STS_3dconv
...
class Bottleneck3d(nn.Module):
    ...
    def __init__(self,
        ...
        self.downsample = downsample
      #  self.temporal_scale = nn.Parameter(torch.tensor([1e-4, 1., 1e-4,]))
        self.relu = build_activation_layer(self.act_cfg)
    ...
    #     b, c, t, h, w = x.size()
    #
    #     conv2_para = conv_3d.conv.weight
    #     outplanes, inplanes = conv2_para.size(0), conv2_para.size(1)
    #     # reshape 3dconv
    #
    #     # reshape 3dconv
    #     temporal_para = conv2_para[0:outplanes // 2, :, :, :, :].view(outplanes // 2, inplanes // groups, -1)
    #     spatial_para = conv2_para[outplanes // 2:, :, :, :, :]
    #     padding_t = (temporal_para.size(-1) - 1) // 2
    #     padding_s = (spatial_para.size(-1) - 1) // 2
    #     if stride == 2:
    #         s = 4
    #     else:
    #         s = 1
    #     out_t1 = einops.rearrange(x, 'b  c  t  h  w  -> (b t) c (h w)', b=b, t=t, h=h, w=w)
    #     out_t2 = einops.rearrange(x, 'b  c  t  h  w  -> (b t) c (w h)',b=b,  t=t, h=h, w=w)
    #     out_spatial = F.conv3d(x, weight=spatial_para, stride=s,
    #                            groups=groups, padding=(padding_t, padding_s, padding_s))
    #
    #     out_1 = F.conv1d(out_t1, weight=temporal_para, stride=s,
    #                      groups=groups, padding=padding_t)
    #     out_2 = F.conv1d(out_t2, weight=temporal_para, stride=s,
    #                      groups=groups, padding=padding_t)
    #
    #     out_1 = einops.rearrange(out_1, '(b t) c (h w)  -> b c t h w',b=b, t=t, h=h // stride,
    #                              w=w // stride)
    #     out_2 = einops.rearrange(out_2, '(b t) c (w h)  -> b c t h w',b=b, t=t, h=h // stride,
    #                              w=w // stride)
    #
    #     out = torch.cat([out_1 + out_2,  out_spatial], dim=1)
    #     out = conv_3d.norm(out)
    #     out = conv_3d.activate(out)
    #     return out
    def reshape_spatial_temporal_conv(self, conv_3d, x):
        if isinstance(conv_3d, nn.Sequential):
            for conv_in in conv_3d:
                if conv_in.conv.weight.size(-1) == 3:
                    conv3d_para = conv_in.conv.weight
                    stride = conv_in.conv.stride
                    padding = conv_in.conv.padding

        else:
            conv3d_para = conv_3d.conv.weight
       ...
       # motion_para = torch.cat([motion_para, motion_para1], dim=0)
       # x_motion = torch.cat([x_motion, x_motion1], dim=1)

        #app_para, app_para1, motion_para = torch.chunk(conv3d_para, 3, dim=0)
        #x_app, x_app1, x_motion = torch.chunk(x, 3, dim=1)
        #app_para = torch.cat([app_para, app_para1], dim=0)
        #x_app = torch.cat([x_app, x_app1], dim=1)


        out_app = self.apperance_pretrain_st(x_app, app_para, stride=stride,
                                             groups=x_app.size(1))
        out_motion = F.conv3d(x_motion, weight=motion_para, stride=stride,
                              groups=x_motion.size(1), padding=padding)
        out = torch.cat([out_app, out_motion], dim=1)

        if isinstance(conv_3d, nn.Sequential):
            for conv_in in conv_3d:
                if conv_in.norm:
                    out = conv_in.norm(out)
                if conv_in.activate:
                    out = conv_in.activate(out)
        return out

    def apperance_pretrain_st(self, x,  app_para, groups=1, stride=[1, 1, 1]):

        b, c, t, h, w = x.size()
        outplanes, inplanes, _, _, _ = app_para.shape
        spatial_para = app_para[:, :, 1:2, :, :]
        temporal_para_row = app_para[:, :, 0, :, :].view(outplanes, inplanes,  1, -1)
        temporal_para_column = app_para[:, :, 2, :, :].view(outplanes, inplanes, 1,  -1)
        padding_s = (spatial_para.size(-1) - 1) // 2
        padding_t = (temporal_para_row.size(-1) - 1) // 2
        if stride[1] == 2:
            s = 4
        else:
            s = 1
        out_t1 = einops.rearrange(x, 'b  c  t  h  w  -> b c t (h w)', t=t, h=h, w=w)
        out_t2 = einops.rearrange(x, 'b  c  t  h  w  -> b c t (w h)', t=t, h=h, w=w)
        out_s = F.conv3d(x, weight=spatial_para, stride=stride,
                         groups=groups, padding=(0, padding_s, padding_s))
        out_t1 = F.conv2d(out_t1, weight=temporal_para_row, stride=(stride[0], s),
                          groups=groups, padding=(0, padding_t))
        out_t2 = F.conv2d(out_t2, weight=temporal_para_column, stride=(stride[0], s),
                          groups=groups, padding=(0, padding_t))
        out_t1 = einops.rearrange(out_t1, 'b  c  t (h  w)  -> b c t h w', t=t // stride[0], h=h // stride[1],
                                  w=w // stride[1])
        out_t2 = einops.rearrange(out_t2, 'b  c  t (w  h)  -> b c t h w', t=t // stride[0], h=h // stride[1],
                                  w=w // stride[1])
        out = out_s + out_t1 + out_t2
        return  out
    ...
    def reshape_temporal_conv(self, conv_3d, x, stride=1, groups=1):
        ...
        # reshape 3dconv
        app_para = conv3d_para[0:outplanes//2, :, :, :, :].view(outplanes//2, inplanes, -1)
        motion_para = conv3d_para[outplanes//2:, :, :, :, :]
        padding_t = (app_para.size(-1) - 1)//2
        padding_s = (motion_para.size(-1) - 1)//2

        out_app = self.apperance_pretrain_temporal(x, app_para, padding_t)
    ...
    def apperance_pretrain_temporal(self, x, app_para, padding_t, groups=1):

        b, c, t, h, w = x.size()
        app_kernel_chunks = torch.chunk(app_para, 2, dim=0)
        #out = einops.rearrange(x, 'b  c  t  h  w  -> (b t) c  h w', t=t, h=h, w=w)
        #out = out.unsqueeze(2)
        #out =  F.conv3d(x, weight=app_para, stride=1, groups=groups, padding=(padding_t, padding_s,
    ...
    def _init_weights(self, pretrained=None):
        ...
        if isinstance(self.pretrained, str):
            ...
            if self.pretrained2d:
                ...
                load_checkpoint(
                    self, self.pretrained, strict=False, logger=logger, map_location='cpu')
